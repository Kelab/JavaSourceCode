 ...
 * 与HashTable主要区别为不支持同步和允许null作为key和value。
 ...
 * 而JDK1.8中，HashMap采用数组+链表+红黑树实现，当链表长度超过阈值8时，将链表转换为红黑树，
 * 这样大大减少了查找时间。
 *
...
public class HashMap<K, V> extends AbstractMap<K, V>
     ...
     * 由于该表使用2的幂次掩码，因此仅在当前掩码上方的位中变化的哈希集将始终
     * 发生冲突。
     * （众所周知的示例是在小表中包含连续整数的Float键集。）
     * 因此，我们应用了一种变换，将向下扩展较高位的影响。
     * 在速度，实用性和位扩展质量之间需要权衡。
     * 由于许多常见的哈希集已经合理分布（因此无法从扩展中受益），
     * 并且由于我们使用树来处理容器中的大量冲突，因此我们仅以最便宜的方式
     * 对一些移位后的位进行XOR，以减少系统损失，以及合并最高位的影响，
            ...
            for (int j = 0; j < oldCap; ++j) {
                ...
                if ((e = oldTab[j]) != null) {
                    ...
                    if (e.next == null)
                        ...
                        //Jdk1.7中对每个节点都进行了reHash
                        //Java8中直接计算hash是否会落在扩容后的一半空间内，一次性做两个链表
                        ...
                        do {
                            next = e.next;
                            //扩容的结果是扩大了一倍，oldCap为10000，若与运算不为0，
                            // 则开头必为1，所以应该放在新链表
                            // 原索引
                            if ((e.hash & oldCap) == 0) {
                                if (loTail == null)
