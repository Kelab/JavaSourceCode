 ...
 *
 * 它包含对集合进行操作的多态算法，“包装器”（包装器），该包装器返回由指定集合支持的新集合，
 * 以及其他一些零碎的结局。
 *
 ...
 *
 * 此类描述应被视为实现说明，而不是规范的一部分。
 *
 * 只要遵守规范本身，实现者就可以随意替换其他算法。
 * （例如， sort使用的算法不必是mergesort，但必​​须是稳定的。）
 *
 * 如果该集合不支持适当的突变原语（例如set方法），则将此类中包含的“破坏性”算法（即修改其操作的集合的算法）
 * 指定为引发UnsupportedOperationException 。
 *
 * 如果调用对集合没有影响，则这些算法可能会（但不是必需）引发此异常。
 * 例如，在已排序的不可修改列表上调用sort方法可能会或可能不会抛出UnsupportedOperationException 。
...
public class Collections {
     ...
     *
     * 在进行此调用之前，必须根据列表元素的自然顺序将其按升序排序（例如通过sort(List)方法）。
     *
     * 如果未排序，则结果不确定。
     *
     * 如果列表包含等于指定对象的多个元素，则不能保证将找到哪个元素。
     *
     * 该方法以log（n）时间运行以获取“随机访问”列表（该列表提供了近恒定时间的位置访问）。
     *
     * 如果指定的列表未实现RandomAccess接口且很大，则此方法将执行基于迭代器的二进制搜索，
     * 该搜索执行O（n）链接遍历和O（log n）元素比较。
     *
     * 二分查找时，ArrayList的效率更高，因为ArrayList支持随机访问，将使用indexedBinarySearch
     * 链表不支持随机访问，使用iteratorBinarySearch
     * @param  <T> the class of the objects in the list
    ...
    /**
     * 洗牌算法，打乱顺序效率不错。
     * 遍历数组，每次生成一个随机的目标位置交换。
     *
     * 使用默认的随机性源随机排列指定的列表。
     * 所有排列发生的可能性几乎相等。
     * 在前述描述中使用“近似”套期，因为默认的随机性源仅是独立选择的比特的无偏源。
     * 如果它是随机选择的位的理想来源，则该算法将选择具有完美一致性的排列。
     * 此实现从最后一个元素到第二个元素向后遍历列表，将随机选择的元素重复交换到“当前位置”。
     * 从列表中从第一个元素到当前位置（包括首尾）的部分中随机选择元素。
     * 此方法以线性时间运行。
     * 如果指定的列表未实现RandomAccess接口并且很大，则此实现将指定的列表转储到数组中，
     * 然后再对其进行混洗，然后将经过改组的数组转储回列表中。
     * 这避免了因将“顺序访问”列表改组到位而导致的二次行为。
     *
     ...
     *
     * 将指定列表中的元素旋转指定距离。
     * 调用此方法后，对于i介于0和list.size（）-1之间（包括两端）的所有值，索引i处的元素将是索引（i-distance）
     * mod list.size（）处的先前元素。
     * （此方法对列表的大小没有影响。）
     * 例如，假设列表包括[t，a，n，k，s] 。调用Collections.rotate（list，1）
     * （或Collections.rotate（list，-4） ）后，list将包含[s，t，a，n，k] 。
     * 请注意，此方法可以有效地应用于子列表，以在保留剩余元素顺序的同时在列表中移动一个或多个元素。
     * 例如，以下成语将索引j处的元素向前移动到位置k （其必须大于或等于j ）：
     *            Collections.rotate(list.subList(j, k+1), -1);
     *
     * 为了使这个具体，假设列表包括[a，b，c，d，e] 。
     * 要将索引1 （ b ）的元素向前移动两个位置，请执行以下调用：
     *            Collections.rotate(l.subList(1, 4), -1);
     *
     * 结果列表为[a，c，d，b，e] 。
     * 要向前移动多个元素，请增加旋转距离的绝对值。 要将元素向后移动，请使用正位移距离。
     * 如果指定的列表很小或实现了RandomAccess接口，则此实现将第一个元素交换到它应该去的位置，
     * 然后反复将置换的元素交换到它应该去的位置，直到将置换的元素交换到第一个元素中。
     * 如有必要，在第二个和后续元素上重复此过程，直到完成旋转。
     * 如果指定的列表很大并且没有实现RandomAccess接口，则此实现将列表分为
     * 两个围绕index- distance mod size的子列表视图。 然后在每个子列表视图上调用reverse(List)方法，
     * 最后在整个列表上调用它。
     *
     * 有关这两种算法的更完整说明，请参见乔恩·本特利的《 Programming Pearls》 （Addison-Wesley，1986年）的2.3节。
     *
